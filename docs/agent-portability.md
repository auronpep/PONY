...
| GitHub Copilot CLI | `.github/plugin/`, `hooks/copilot-hooks.json`, `commands/`, `skills/`, `AGENTS.md`, `.github/copilot-instructions.md`, `~/.copilot/copilot-instructions.md` | Plugin-supported (`copilot plugin marketplace add DietrichGebert/ponytail` + `copilot plugin install ponytail@ponytail`). Fallback instruction mode remains: per-project from `AGENTS.md` or `.github/copilot-instructions.md`, or globally from `~/.copilot/copilot-instructions.md` (instruction-tier, no `/ponytail` levels or hooks). |
| Antigravity | `.agents/plugins/marketplace.json`, `gemini-extension.json`, `commands/`, `skills/`, `AGENTS.md` | Installs the extension (`agy plugin install`), reusing `gemini-extension.json`; it converts the `/ponytail` commands into skills you type into the chat rather than pick from a slash menu. Also reads `AGENTS.md` at the repo root as always-on rules (like `.cursorrules`/`CLAUDE.md`); `.agents/rules/` works for workspace rules. |
...
| OpenClaw | `.openclaw/skills/`, `skills/`, `scripts/build-openclaw-skills.js` | Skill package generated from `skills/` (one-line descriptions, body copied verbatim). Install via `clawhub install ponytail` — review, audit, debt, and help install the same way; without ClawHub, copy `.openclaw/skills/ponytail` into `~/.openclaw/skills/`. Also exposes `/ponytail`. |
| Kiro | `.kiro/steering/ponytail.md` | Steering rule; copy globally or into a project. |
| Aider | `AGENTS.md` | Reads `AGENTS.md` as conventions. Instruction-tier. |
